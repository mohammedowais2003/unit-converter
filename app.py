...
# Apply custom CSS with dark/black labels
st.markdown(
    """
    <style>
    :root {
    ...
    if from_unit == to_unit:
        ...
        try:
            ...
            st.markdown(
                ...
                f"<div style='font-size: 1.1rem; margin-bottom: 0.5rem; color: var(--dark);'>Conversion Result</div>"
                f"<div style='font-size: 1.5rem; color: var(--dark);'>{value:.6g} {from_unit} = {result:.6g} {to_unit}</div>"
                f"</div>", 
                unsafe_allow_html=True
            )
